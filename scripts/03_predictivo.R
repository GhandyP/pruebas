...
log_msg("INFO", sprintf("03_predictivo run=%s horizonte=%d asof=%s",
...
riesgo_resumen <- stats::aggregate(
  cbind(n = rep(1, nrow(pos)),
        margin_total = pos$margin_used,
        notional_total = pos$notional_inv) ~ risk_label,
...
resumen <- c(
  ...
  sprintf("- **Hist\u00f3rico:** %s \u2192 %s (%d d\u00edas)",
          format(min(diario$date)),
          format(max(diario$date)),
          nrow(diario)),
  sprintf("- **Pron\u00f3stico total:** %.2f en 30 d\u00edas (media %.2f/d\u00eda).",
          sum(pred_diaria),
          mean(pred_diaria)),
  sprintf("- **Intervalo 95%% total:** %.2f \u2192 %.2f.",
          sum(ci_low), sum(ci_high)),
  sprintf("- **Modelo:** %s.",
          if (inherits(modelo, "forecast")) "auto.arima(seasonal=FALSE)" else
            "arima(1,1,1)"),
  "",
  "## Riesgo de margen",
  "",
  ...
  "",
  sprintf("_Generado por 03_predictivo.R \u2014 run %s \u2014 asof %s._",
          run_id, format(asof))
)
...
escribir_metadata(
  ...
  params = list(input = ops_path, posiciones = pos_path,
