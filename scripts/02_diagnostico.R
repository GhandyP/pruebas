...
parser <- construir_parser(list(
  ...
  list(name = "precios", help = "CSV de precios (precios.csv)"),
  list(name = "base", help = "Mes base (YYYY-MM-01)"),
  list(name = "target", help = "Mes objetivo (YYYY-MM-01)"),
...
log_msg("INFO", sprintf(
  "02_diagnostico run=%s base=%s target=%s asof=%s",
...
driver_mesa <- function(d_base, d_tgt) {
  agg <- function(d) {
    if (nrow(d) == 0) {
      return(data.frame(desk = DESKS, commissions = 0))
    }
...
driver_simbolo <- function(d_base, d_tgt) {
  agg <- function(d) {
    if (nrow(d) == 0) {
      return(data.frame(symbol = character(), commissions = numeric()))
    }
    if (requireNamespace("dplyr", quietly = TRUE)) {
      d |>
      ...
      names(out) <- c("symbol", "commissions")
      out
    }
  }
  ab <- agg(d_base)
...
vol_sym <- function(pr, desde, hasta) {
  ...
  if (requireNamespace("dplyr", quietly = TRUE)) {
    pp |>
      dplyr::group_by(symbol) |>
      dplyr::summarise(
        vol_diaria = stats::sd(close / dplyr::lag(close) - 1, na.rm = TRUE),
        .groups = "drop"
      )
  } else {
    ...
    if (length(parts) == 0) {
      return(data.frame(
        symbol = character(), vol_diaria = numeric(),
        stringsAsFactors = FALSE
      ))
    }
...
resumen <- c(
  ...
  "",
  sprintf(
    "_Generado por 02_diagnostico.R \u2014 run %s \u2014 asof %s._",
    run_id, format(asof)
  )
)
...
escribir_metadata(
  ...
  params = list(
    input = ops_path, precios = precios_path,
